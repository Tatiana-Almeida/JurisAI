...
| SEC-009 | RAG poderia vazar documentos entre tenants ou enviar conteudo para provider externo | alto | `knowledge_base` indexa, pesquisa, reindexa e lista `IndexingJob` apenas por `organization`; `ask` retorna fontes explicitas, `confidence` e `retrieval_method`; nesta fase nao envia documentos para nenhum provider externo | Preservar filtros por tenant, manter respostas sem invencao de conteudo e so evoluir para embeddings externos com controles explicitos, consentimento e configuracao por tenant | [CONFIRMADO_NO_CODIGO] mitigado parcialmente nas fases 1 e 2 do `knowledge_base` |
