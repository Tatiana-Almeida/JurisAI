...
| SEC-009 | RAG poderia vazar documentos entre tenants ou enviar conteudo para provider externo | alto | `knowledge_base` agora indexa e pesquisa apenas por `organization`; `ask` retorna fontes explicitas e nesta fase nao envia documentos para nenhum provider externo | Preservar filtros por tenant, manter respostas sem invencao de conteudo e so evoluir para embeddings externos com controles explicitos | [CONFIRMADO_NO_CODIGO] mitigado parcialmente na implementacao inicial do `knowledge_base` |
...
- [CONFIRMADO_NO_CODIGO] A fase atual do `knowledge_base` usa apenas busca textual local, indexacao de chunks no banco relacional e respostas com `sources`; nenhum documento da organizacao e enviado a provider externo nesta etapa.
